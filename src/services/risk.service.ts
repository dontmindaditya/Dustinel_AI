...
const DEDUCTIONS = {
  ...
  HAZARD_EACH: 5,
  HAZARD_MAX: 20,
  FATIGUE_HIGH: 15,  // fatigueScore > 0.7
  FATIGUE_MED: 8,   // fatigueScore > 0.5
  NIGHT_SHIFT: 5,
  PREVIOUS_SCORE_LOW: 5,  // last score < 60
  CHRONIC_CONDITION: 5,
} as const;
...
export function calculateRuleBasedScore(params: {
  ...
  if (mandatoryPPE.includes("mask") && !vision.face.hasMask) {
    ...
    riskFactors.push({ type: "NO_MASK", severity: "HIGH", weight: 0.35, confidence: vision.face.confidence });
  }
  if (mandatoryPPE.includes("helmet") && !vision.face.hasHelmet) {
    score -= DEDUCTIONS.NO_HELMET;
    riskFactors.push({ type: "NO_HELMET", severity: "HIGH", weight: 0.30, confidence: vision.face.confidence });
  }

  // Environment hazards
  if (vision.environment.dustLevel === "EXTREME") {
    score -= DEDUCTIONS.DUST_EXTREME;
    riskFactors.push({ type: "DUST_LEVEL_EXTREME", severity: "HIGH", weight: 0.20 });
  } else if (vision.environment.dustLevel === "HIGH") {
    score -= DEDUCTIONS.DUST_HIGH;
    riskFactors.push({ type: "DUST_LEVEL_ELEVATED", severity: "MEDIUM", weight: 0.10 });
  }

  if (vision.environment.lightingLevel === "LOW") {
    score -= DEDUCTIONS.POOR_LIGHTING;
    riskFactors.push({ type: "POOR_LIGHTING", severity: "MEDIUM", weight: 0.10 });
  }
  ...
  if (hazardDeduction > 0) {
    ...
    vision.environment.detectedHazards.forEach((hazard) => {
      riskFactors.push({ type: `HAZARD_${hazard.toUpperCase()}`, severity: "MEDIUM", weight: 0.05 });
    });
  }
  ...
  if (vision.face.fatigueScore > 0.7) {
    ...
    riskFactors.push({ type: "HIGH_FATIGUE", severity: "HIGH", weight: 0.15, confidence: vision.face.fatigueScore });
  } else if (vision.face.fatigueScore > 0.5) {
    score -= DEDUCTIONS.FATIGUE_MED;
    riskFactors.push({ type: "MODERATE_FATIGUE", severity: "MEDIUM", weight: 0.08, confidence: vision.face.fatigueScore });
  }

  if (shiftType === "night") {
    score -= DEDUCTIONS.NIGHT_SHIFT;
    riskFactors.push({ type: "NIGHT_SHIFT", severity: "LOW", weight: 0.05 });
  }

  if (previousScore !== undefined && previousScore < 60) {
    score -= DEDUCTIONS.PREVIOUS_SCORE_LOW;
    riskFactors.push({ type: "PREVIOUS_SCORE_LOW", severity: "LOW", weight: 0.05 });
  }

  if (worker.healthProfile.conditions.length > 0) {
    score -= DEDUCTIONS.CHRONIC_CONDITION;
    riskFactors.push({ type: "CHRONIC_CONDITION", severity: "LOW", weight: 0.05 });
  }
...
export function buildMLAnalysis(
  score: number,
